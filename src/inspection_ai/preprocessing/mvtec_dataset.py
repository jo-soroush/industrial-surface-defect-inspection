...
class MVTecAnomalyDataset(torch.utils.data.Dataset):
    """Thin Dataset wrapper for governed MVTec anomaly entries."""
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        image_path = _require_entry_string(entry, "image_path")
        raw_label = _require_entry_string(entry, "label")
        label_id = entry.get("label_id")
        if isinstance(label_id, bool) or not isinstance(label_id, int):
            raise ValueError("Dataset entry field label_id must be an integer.")

        image = load_and_preprocess_image(image_path, self.preprocessing_config)
